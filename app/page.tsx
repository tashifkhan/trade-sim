...
import { OrderbookData, SimulationParams, SimulationResults } from '@/types/types';
...
export default function Home() {
  ...
  const wsUrl = process.env.NEXT_PUBLIC_WS_URL || 'wss://ws.okx.com:8443/ws/v5/public';
  const { data: orderbook, latency, status } = useWebSocket(wsUrl, params.symbol);

  useEffect(() => {
    if (params.run && orderbook && orderbook.bids.length > 0 && orderbook.asks.length > 0) {
      ...
      const impactBps = calculateImpact(quantityCrypto, 10000, params.volatility);
  ...
  return (
    <Container maxWidth={false} sx={{ height: '100vh', py: 4 }}>
      ...
      >
        <Box sx={{ width: { xs: '100%', md: '30%' }, minWidth: { md: '300px' } }}>
          <InputPanel 
            params={params} 
